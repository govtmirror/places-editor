var tileMath = {
  ...
  long2tile: function (lon, zoom, pixelOffset, decimal) {
    var returnValue = ((lon + 180) / 360 * Math.pow(2, zoom) + (pixelOffset ? pixelOffset / 256 : 0));
    return decimal ? returnValue : Math.floor(returnValue);
  },
  lat2tile: function (lat, zoom, pixelOffset, decimal) {
    var returnValue = (((1 - Math.log(Math.tan(lat * Math.PI / 180) + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom)) + (pixelOffset ? pixelOffset / 256 : 0));
    return decimal ? returnValue : Math.floor(returnValue);
  },
