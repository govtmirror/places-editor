...
css += ".preset-icon-npmap-symbol-library{background-image:url(img/npmap-symbol-library-sprite.png);background-repeat:no-repeat;width:24px;height:24px;}\n";
...
var images = {'maki': {}, 'npmap-symbol-library': {}};

_.forEach(maki, function(dimensions, name) {
    var match = name.match(/(.*)-(12|18|24)/),
        name = match[1],
        size = match[2],
        group = images.maki[name] = images.maki[name] || {};
...
_.forEach(npmap, function(dimensions, name) {
    var match = name.match(/(.*)-(12|18|24)/),
        name = match[1],
        size = match[2],
        group = images['npmap-symbol-library'][name] = images['npmap-symbol-library'][name] || {};
    group[size] = [dimensions.x, dimensions.y];
